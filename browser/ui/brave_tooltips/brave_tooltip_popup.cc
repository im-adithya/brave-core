...
gfx::Point BraveTooltipPopup::GetOriginForSize(const gfx::Size& size) {
#if 0
  if (!profile_->GetPrefs()->HasPrefPath(
          prefs::kAdTooltipLastScreenPositionX) ||
      !profile_->GetPrefs()->HasPrefPath(
          prefs::kAdTooltipLastScreenPositionY)) {
...
void BraveTooltipPopup::SaveOrigin(const gfx::Point& origin) const {
#if 0
  profile_->GetPrefs()->SetInteger(prefs::kAdTooltipLastScreenPositionX,
                                   origin.x());
  profile_->GetPrefs()->SetInteger(prefs::kAdTooltipLastScreenPositionY,
                                   origin.y());
#endif
}
